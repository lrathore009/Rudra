...
export function ResearchLibraryPanel() {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      {trends && (
        <div className="flex items-center gap-2 font-terminal text-[9px] text-muted-foreground">
      ...
      >
        <input
          ...
          className="min-w-0 flex-1 rounded border border-primary/20 bg-secondary/80 px-2 py-0.5 font-terminal text-[10px] text-foreground outline-none placeholder:text-muted-foreground/60"
        />
        <button
          type="submit"
          className="border border-primary/25 px-1.5 py-0.5 font-terminal text-[8px] uppercase text-muted-foreground hover:bg-primary/10"
      ...
      <div className="min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5">
        {reports.length === 0 ? (
          <p className="font-terminal text-[10px] text-muted-foreground/80">No reports yet — run /research or save via agent.</p>
        ) : (
          reports.map((r) => (
            <div key={r.id} className="rounded border border-primary/10 px-2 py-1">
              <div className="flex items-start gap-1.5">
                <BookOpen className="mt-0.5 h-3 w-3 shrink-0 neon-dim" />
                <div className="min-w-0">
                  <p className="truncate font-terminal text-[10px] text-foreground/90">{r.title}</p>
                  <p className="font-terminal text-[8px] text-muted-foreground/90">
                    conf {r.confidence_score.toFixed(2)}
                    {"score" in r && r.score != null ? ` · rrf ${r.score.toFixed(3)}` : ""}
                  </p>
                  {"snippet" in r && r.snippet ? (
                    <p className="line-clamp-2 font-terminal text-[9px] leading-tight text-muted-foreground/90">{r.snippet}</p>
                  ) : null}
                </div>
              </div>
            </div>
      ...
      <form
        className="flex gap-1 border-t border-primary/10 pt-1"
        onSubmit={(e) => {
          e.preventDefault();
          void addTopic();
        }}
      ...
      {spokenText ? (
        <p className="line-clamp-3 border-t border-primary/10 pt-1 font-terminal text-[9px] leading-relaxed text-muted-foreground">
          {spokenText}
        </p>
      ) : null}
    </div>
